...
# Define file paths
DATA_DIR = "../.."
WATER_FILE = os.path.join(DATA_DIR, "raw_data", "water_b_tommerby", "tommerby_CombinedDataframe_SummedAveraged_withoutOutliers.csv")
WEATHER_FILE = os.path.join(DATA_DIR, "raw_data", "weather", "Weather_Bronderslev_20152022.csv")
OUTPUT_FILE = "tommerby_water_weather_aligned.csv"

def align_data_tommerby():
    ...
    df_water = pd.read_csv(WATER_FILE, header=0, names=['timestamp', 'water_consumption'])
    ...
    print(f"Water Data Loaded. Shape: {df_water.shape}")
    print(df_water.head())
    ...
    # but here we parsed it. Let's ensure it's timezone aware and matches water data).
    ...
    # or 'left' if we want to keep all water data rows. 
    # Given the goal is forecasting water with weather as covariate, we need weather for every water point.
    # Inner join is safest to ensure complete data, but let's check if we lose much.
    
    df_aligned = df_water.join(df_weather, how='inner')
