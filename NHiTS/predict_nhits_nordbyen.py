"""
NHiTS Model Prediction Script for Nordbyen Heat Consumption Forecasting.

This script:
1. Loads the trained NHiTS model
...
def predict_next_horizon(
    ...
    """
    Load the trained NHiTS model and preprocessing state, and predict the next `n` hours.
    
    Returns a probabilistic TimeSeries (with samples) if num_samples > 1.
    """
    print("=" * 70)
    print("NHiTS PREDICTION - NORDBYEN HEAT CONSUMPTION")
    print("=" * 70)
    ...
    # 5. Load the trained NHiTS model
    print("\n[5/6] Loading trained NHiTS model...")
    model = NHiTSModel.load(model_path)
    print(f"  ✓ Model loaded")

    # Merge future into past for NHiTS
    # Note: NHiTS only uses past info, so we only need future covariates up to T (current time)
    # We ignore the future part (T+1...T+n) because NHiTS doesn't support future covariates.
    
    future_scaled_up_to_T = future_scaled[:-n]
    
    if future_scaled_up_to_T:
        if past_scaled:
            past_scaled = past_scaled.stack(future_scaled_up_to_T)
        else:
            past_scaled = future_scaled_up_to_T
        future_scaled = None
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(DATA_DIR, "nhits_predictions_future_24h.csv")
