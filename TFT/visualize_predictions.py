...
This script loads evaluation results (metrics CSV and predictions CSV) from the
...
def load_results(results_dir: str):
    """Load metrics and predictions CSV files from the given directory."""
...
def plot_actual_vs_predicted(df: pd.DataFrame, save_path: str):
    ...
    ax.set_title("TFT Model: Actual vs Predicted")
    ax.legend()
    fig.tight_layout()
...
def plot_daily_comparison(df: pd.DataFrame, save_path: str, day_offset: int = 10):
    ...
    ax1.set_title(f"Day {day_offset} Pattern")
    ax1.legend()
    # error bar plot
...
if __name__ == "__main__":
    ...
    # generate plots
    plot_actual_vs_predicted(preds, os.path.join(RESULTS_DIR, "plot_actual_vs_predicted.png"))
    plot_error_distribution(preds, os.path.join(RESULTS_DIR, "plot_error_distribution.png"))
    plot_scatter(preds, metrics, os.path.join(RESULTS_DIR, "plot_scatter.png"))
    plot_daily_comparison(preds, os.path.join(RESULTS_DIR, "plot_daily_pattern.png"))
    plot_error_over_time(preds, os.path.join(RESULTS_DIR, "plot_error_over_time.png"))
